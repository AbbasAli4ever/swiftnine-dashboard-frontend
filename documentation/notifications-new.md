...
- `notifications:init` — initial payload (array)
  ...
  - Includes DB fields such as `userId` and `readAt`.
...
All state APIs return the updated notification object (enriched with `taskId`/`taskName`/`commentName`) on success. Example responses below reflect the runtime shape returned by the API (these responses include `userId` and `readAt` when present).
