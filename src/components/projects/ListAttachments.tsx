...
import {
  taskListAttachmentService,
  TaskListAttachment,
  TaskListFileAttachment,
} from "@/services/task-list-attachment.service";
...
// EditAttachmentModal expects ProjectAttachment shape — adapter to reuse it
function toProjectAttachmentShape(att: TaskListAttachment): ProjectAttachment {
  if (att.kind === "FILE") {
    return att as unknown as ProjectAttachment;
  }
  return att as unknown as ProjectAttachment;
}

export default function ListAttachments({ listId }: ListAttachmentsProps) {
  const [attachments, setAttachments] = useState<TaskListAttachment[]>([]);
  ...
  const [viewingImage, setViewingImage] = useState<TaskListFileAttachment | null>(null);
  const [editingAttachment, setEditingAttachment] = useState<TaskListAttachment | null>(null);
  const [addLinkOpen, setAddLinkOpen] = useState(false);
  const inputRef = useRef<HTMLInputElement>(null);

  const fetchAttachments = useCallback(async (opts?: { cursor?: string }) => {
    const isLoadMore = !!opts?.cursor;
    if (isLoadMore) setLoadingMore(true);
    else setLoading(true);

    try {
      const result = await taskListAttachmentService.list(listId, {
        cursor: opts?.cursor,
        limit: 30,
      });
      if (isLoadMore) {
  ...
  const uploadFile = async (file: File) => {
    ...
    try {
      ...
      const { uploadUrl, s3Key } = await taskListAttachmentService.presign(listId, {
        fileName: file.name,
        mimeType,
        fileSize: file.size,
      });
      ...
      await taskListAttachmentService.confirm(listId, {
        s3Key,
        fileName: file.name,
        mimeType,
        fileSize: file.size,
  ...
  const handleAddLink = async (payload: { linkUrl: string; title: string; description?: string }) => {
    try {
      await taskListAttachmentService.createLink(listId, payload);
  ...
  const handleEdit = async (payload: { title: string | null; description: string | null }) => {
    ...
    try {
      const updated = await taskListAttachmentService.update(listId, editingAttachment.id, payload);
  ...
  // ── Delete ────────────────────────────────────────────────────────────────
  const handleDelete = async (att: TaskListAttachment) => {
    setDeletingId(att.id);
    try {
      await taskListAttachmentService.delete(listId, att.id);
  ...
  const handleOpenAttachment = (att: TaskListAttachment) => {
    if (att.kind === "FILE") {
  ...
  return (
    <>
      ...
      <EditAttachmentModal
        ...
        attachment={editingAttachment ? toProjectAttachmentShape(editingAttachment) : null}
        onSubmit={handleEdit}
      />
        ...
        <div className="mb-5 flex flex-wrap items-center gap-3">
          <div className="ml-auto flex items-center gap-2">
            ...
            <button
              ...
              className="flex items-center gap-1.5 rounded-xl bg-brand-500 dark:bg-gray-000 dark:text-black px-3 py-2 text-xs font-medium text-white hover:bg-brand-600 dark:hover:bg-gray-200 transition-colors"
            >
          ...
          {isEmpty && (
            <div className="flex flex-col items-center justify-center py-20 text-center">
              ...
              <p className="mt-1.5 max-w-xs text-sm text-gray-400">
                Upload files or add external links to keep resources organized in this list.
              </p>
              <div className="mt-5 flex items-center gap-3">
                <button
                  type="button"
                  onClick={() => inputRef.current?.click()}
                  className="flex items-center gap-2 rounded-xl bg-brand-500 dark:bg-gray-000 dark:text-black  px-4 py-2 text-sm font-medium text-white hover:bg-brand-600 dark:hover:bg-gray-200 transition-colors"
                >
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      {u.confirmFailed && (
                        <p className="mt-1 text-[11px] text-red-500">
                          Failed to save.{" "}
                          <button type="button" onClick={() => void retryConfirm(u)} className="underline hover:no-underline">
                            Retry
                          </button>
                        </p>
                      )}
